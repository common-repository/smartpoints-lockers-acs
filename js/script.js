...
const elementInputAddress = '#billing_address_1';
const elementInputCity = '#billing_city';
const pluginFolder = smartpoints_lockers_acs_plugin_data.plugin_url; // Use localized script data for plugin URL
            ...
            return function () {
                ...
                infoWindow.setContent(
                    `<div class="acs-sp-map-point-infobox">
                        ...
                        <button class="selected" counter="${counter}">Επιλογή</button>
                    </div>`
                );
...
jQuery(document).on('click', '.selected', function (e) {
    ...
    jQuery(".locker-container").html(
        `<div class="mg-selected-locker-title">Έχετε επιλέξει το:</div>
...
function fetchLocations() {
    jQuery.ajax({
        dataType: "json",
        url: pluginFolder + "/data.json?v=" + new Date().getTime(),
        async: false,
...
function findNearestPoint(lat, lng, is_address = true) {
    ...
    if (distanceText <= 3000) {
        ...
        text += '<img src="' + pluginFolder + '/icons/point.svg" alt="point-icon" />'
        if (is_address) {
            text += "Το Πλησιέστερο ACS Point είναι: <strong>" + (distanceText) + "</strong>m"
        } else {
            text += "Φαίνεται ότι υπάρχει ACS Point στην περιοχή σας!";
        }
    }
